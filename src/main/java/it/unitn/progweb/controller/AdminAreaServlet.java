...
public class AdminAreaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String sqlUserReservation = "SELECT DISTINCT u.uid, u.username, m.title, r.id AS reservation, s.date_time AS data, st.column, st.row, th.description as sala  FROM ((((reservation r join \"user\" u ON r.user_id=u.uid) JOIN show s ON s.id=r.show_id) JOIN seat st ON st.id = r.seat_id) JOIN movie m ON s.movie_id=m.id) JOIN theater th ON th.id=s.theater_id  WHERE s.date_time > current_timestamp ORDER BY u.uid;";
